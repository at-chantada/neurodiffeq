...
class Solution1D_bundle(BaseSolution):
    def _compute_u(self, net, condition, *ts):
        return condition.enforce(net, *ts)
...
class Solver1D_bundle(BaseSolver):
    ...
    def __init__(self, ode_system, conditions, t_min, t_max,
                 parameters_min=None, parameters_max=None,
                 nets=None, train_generator=None, valid_generator=None, analytic_solutions=None, optimizer=None,
                 criterion=None, n_batches_train=1, n_batches_valid=4, metrics=None, n_output_units=1,
                 # deprecated arguments are listed below
                 batch_size=None, shuffle=None):
        ...
        if isinstance(parameters_min, float) or isinstance(parameters_min, int):
            r_min = (t_min,) + (parameters_min,)

        if isinstance(parameters_max, float) or isinstance(parameters_max, int):
            r_max = (t_max,) + (parameters_max,)

        if parameters_min is None and parameters_max is None:
            r_min = (t_min,)
            r_max = (t_max,)

        if isinstance(parameters_min, tuple) and isinstance(parameters_max, tuple):
            r_min = (t_min,) + parameters_min
            r_max = (t_max,) + parameters_max
        ...
        super(Solver1D_bundle, self).__init__(
            diff_eqs=ode_system,
            conditions=conditions,
            nets=nets,
            train_generator=train_generator,
    ...
    def get_solution(self, copy=True, best=True):
        ...
        return Solution1D_bundle(nets, conditions)

    def _get_internal_variables(self):
        available_variables = super(Solver1D_bundle, self)._get_internal_variables()
        available_variables.update({
            'r_min': self.r_min,
            'r_max': self.r_max,
        })
